...
def randomGraph(nodes_number: int, dim_node_label: int, dim_arc_label: int, dim_target: int, density: float,
                *, normalize_features: bool = False, aggregation_mode: str = 'average',
                problem_based: str = 'n') -> CompositeGraphObject:
    ...
    :param aggregation_mode: (str) in ['average','normalized','sum']. Default 'average'. Go to CompositeGraphObject.buildArcNode() for details
    ...
    # RETURN GRAPH
    return CompositeGraphObject(arcs=arcs, nodes=nodes, targets=targs, problem_based=problem_based,
                       output_mask=output_mask, aggregation_mode=aggregation_mode)


# ---------------------------------------------------------------------------------------------------------------------
def simple_graph(problem_based: str, aggregation_mode: str = 'average') -> CompositeGraphObject:
    """ return a single simple CompositeGraphObject for debugging purpose """
    ...
    return CompositeGraphObject(arcs=arcs, nodes=nodes, targets=targs, problem_based=problem_based, aggregation_mode=aggregation_mode)
...
# ---------------------------------------------------------------------------------------------------------------------
def getSet(glist: list[CompositeGraphObject], set_indices: list[int], problem_based: str, aggregation_mode: str,
           verbose: bool = False) -> list[CompositeGraphObject]:
    """ get the Set from a dataset given its set of indices

    :param glist: (list of CompositeGraphObject or str) dataset from which the set is picked
    ...
    return [CompositeGraphObject.load(i, problem_based=problem_based, aggregation_mode=aggregation_mode) for i in setlist]


# ---------------------------------------------------------------------------------------------------------------------
def getbatches(glist: list[CompositeGraphObject], problem_based: str, aggregation_mode: str, batch_size: int = 32, number_of_batches=None,
               one_graph_per_batch=True) -> Union[list[CompositeGraphObject], list[list[CompositeGraphObject]]]:
    """ Divide the Set into batches, in which every batch is a CompositeGraphObject or a list of CompositeGraphObject

    :param glist: (list of CompositeGraphObject) to be split into batches
    ...
    :param one_graph_per_batch: (bool) if True, all graphs belonging to a batch are merged to form a single CompositeGraphObject
    ...
    if one_graph_per_batch: batches = [CompositeGraphObject.merge(i, problem_based=problem_based, aggregation_mode=aggregation_mode) for i in
                                       batches]
    return batches


# ---------------------------------------------------------------------------------------------------------------------
def normalize_graphs(gTr: Union[CompositeGraphObject, list[CompositeGraphObject]], gVa: Optional[Union[CompositeGraphObject, list[CompositeGraphObject]]],
                     gTe: Optional[Union[CompositeGraphObject, list[CompositeGraphObject]]], based_on: str = 'gTr',
    ...
    :param gTr: (CompositeGraphObject or list of GraphObjects) for Training Set
    :param gVa: (CompositeGraphObject or list of GraphObjects or None) for Validation Set
    :param gTe: (CompositeGraphObject or list of GraphObjects or None) for Test Set
    :param based_on: (str) in ['gTr','all']. If 'gTr', only gTr data is used; if 'all', entire dataset data is used
    """

    def checktype(g: Union[list[CompositeGraphObject], None], name: str):
        """ check g: it must be a GraphObect or a list of GraphObjects """
        if g is None: return list()
        if not (type(g) == CompositeGraphObject or (type(g) == list and all(isinstance(x, CompositeGraphObject) for x in g))):
            raise TypeError(f'type of param <{name}> must be CompositeGraphObject or list of Graphobjects')
        return g if type(g) == list else [g]

    # check if inputs are CompositeGraphObject OR list(s) of CompositeGraphObject(s) and the normalization method
    ...
    G = CompositeGraphObject.merge(gTr, problem_based='n', aggregation_mode='sum')
    if based_on == 'all': G = CompositeGraphObject.merge(G + gTe + gVa, problem_based='n', aggregation_mode='sum')
...
# ---------------------------------------------------------------------------------------------------------------------
def prepare_LKO_data(dataset: Union[CompositeGraphObject, list[CompositeGraphObject], list[list[CompositeGraphObject]]],
        ...
        -> tuple[Union[list[CompositeGraphTensor], list[list[CompositeGraphTensor]]], list[CompositeGraphTensor], Optional[list[CompositeGraphTensor]]]:
    """ Prepare dataset for Leave K-Out procedure. The output of this function must be passed as arg[0] of model.LKO() method.
    :param dataset: a single CompositeGraphObject OR a list of CompositeGraphObject OR list of lists of CompositeGraphObject on which <gnn> has to be valuated
                    > NOTE: for graph-based problem, if type(dataset) == list of CompositeGraphObject,
    ...
    :param aggregation_mode: (str) for aggregation method during dataset creation. See CompositeGraphObject for details."""
    ...
    if isinstance(dataset, CompositeGraphObject):
        zero_mask = np.zeros(len(dataset.set_mask), dtype=bool)

        # normalization procedure - available only on CompositeGraphObject
        if normalize_method: normalize_graphs(dataset, None, None, based_on=normalize_method)

        # convert CompositeGraphObject to CompositeGraphTensor
        dataset = CompositeGraphTensor.fromGraphObject(dataset)
    ...
    # MULTI GRAPH CASE: dataset is a list of graphs or a list of lists of graphs. :param dataset_ for details
        ...
        if all(isinstance(i, CompositeGraphObject) for i in dataset): dataset = [dataset]
        assert all(len(i) > number_of_batches for i in dataset)
        assert all(isinstance(i, list) for i in dataset) and all(isinstance(j, CompositeGraphObject) for i in dataset for j in i)
        ...
        # Final dataset for LKO procedure: merge graphs belonging to classes/dataset to obtain 1 CompositeGraphObject per batch
        dataset = [CompositeGraphObject.merge(i, problem_based=problem_based, aggregation_mode=aggregation_mode) for i in flattened]

        # Transform all the GraphObjects in GraphTensors
        # dataset = [CompositeGraphTensor.fromGraphObject(g) for g in dataset]
        ...
        for i in range(len(dataset)):
            ...
            # append batch graphs
            # CompositeGraphObject->CompositeGraphTensor conversion is here because of the normalization procedure which is available only on GraphObjects
            gTRs.append([CompositeGraphTensor.fromGraphObject(g) for g in gTr])
            gTEs.append(CompositeGraphTensor.fromGraphObject(gTe))
            gVAs.append(CompositeGraphTensor.fromGraphObject(gVa) if gVa is not None else None)

    else:
        raise TypeError('param <dataset> must be a CompositeGraphObject, a list of GraphObjects or a list of lists of Graphobjects')
