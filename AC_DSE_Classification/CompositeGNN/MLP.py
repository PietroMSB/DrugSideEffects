...
# ---------------------------------------------------------------------------------------------------------------------
def get_inout_dims2(net_name: str, dim_node_label: int, dim_arc_label: int, dim_target: int, problem_based: str, dim_state: int,
    ...
    if net_name == 'state':
        input_shape = [i + NLgen + AL + 2 * DS for i in NL]  # AL + 2 * (NL + DS)
        output_shape = DS
    ...
    elif isinstance(hidden_units, int): hidden_units = [hidden_units]
...
def get_inout_dims(net_name: str, dim_node_label: int, dim_arc_label: int, dim_target: int, problem_based: str, dim_state: int,
    ...
    # hidden layers
    if hidden_units is None: hidden_units = []
    elif isinstance(hidden_units, int): hidden_units = [hidden_units]
    layers = list(hidden_units) + [output_shape]
