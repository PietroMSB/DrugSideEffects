...
path_results = "Results/GNN_Transduction_"+run_id+".txt"
...
netOut = utils.MLP(input_dim=20, layers=[20, 2], activations=['tanh', 'softmax'],
                      ...
                      dropout_percs=[0, 0],
                      dropout_pos=[0, 0])
model = GNNedgeBased(netSt, netOut, optimizer = tf.keras.optimizers.Adam(LR), loss_function = tf.nn.softmax_cross_entropy_with_logits, state_vect_dim = STATE_DIM, max_iteration=MAX_ITER, addressed_problem='c', loss_arguments=None, output_activation=tf.nn.softmax, threshold=0.0001)
