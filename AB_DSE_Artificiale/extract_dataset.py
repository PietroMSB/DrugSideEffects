...
#parameters
side_effects_occurrences_minimum = 0
...
#acquire data
ppi_a = pandas.read_table("Datasets/D1/Raw/HI-union.tsv")
dpi_a = pandas.read_table("Datasets/D1/Raw/dpi_preprocessed.tsv", dtype=np.unicode_)
dsa_aaa = pandas.read_table("Datasets/D1/Raw/meddra_all_se.tsv")
...
for d in unique_drugs_dsa:
	if str(int(d[4:])) not in unique_drugs_dpi:
		discarded += 1
		continue
...
for i in range(dpi_a.values.shape[0]):
	...
	transformed_drug_id = ""
	if int(dpi_a.values[i][0]) < 100:
		transformed_drug_id = "CID1000000"+dpi_a.values[i][0]
	else:
		transformed_drug_id = "CID100000"+dpi_a.values[i][0]
	if (transformed_drug_id not in final_drugs):
		del_indices.append(i)
...
#save output files
out_file = open("Datasets/D1/Output/drugs.pkl", 'wb')
pickle.dump(final_drugs, out_file)
out_file.close()
out_file = open("Datasets/D1/Output/genes.pkl", 'wb')
pickle.dump(final_proteins, out_file)
out_file.close()
out_file = open("Datasets/D1/Output/side_effects.pkl", 'wb')
pickle.dump(final_se, out_file)
out_file.close()
out_file = open("Datasets/D1/Output/drug_gene_links.pkl", 'wb')
pickle.dump(final_dpi, out_file)
out_file.close()
out_file = open("Datasets/D1/Output/gene_gene_links.pkl", 'wb')
pickle.dump(final_ppi, out_file)
out_file.close()
out_file = open("Datasets/D1/Output/drug_side_effect_links.pkl", 'wb')
