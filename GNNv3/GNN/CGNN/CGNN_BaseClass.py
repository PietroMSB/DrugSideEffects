...
import GNNv3.GNN.GNN_metrics as mt
...
class BaseCGNN(ABC):
	...
	@abstractmethod
	def set_weights(self, weights_state: Union[list[list[array]], list[list[list[array]]]], weights_output: Union[list[array], list[list[array]]]) -> None:
		pass

	@abstractmethod
	def Loop(self, g: CompositeGraphObject, *, training: bool = False) -> tuple[int, tf.Tensor, tf.Tensor]:
		pass
	...
	## EVALUATE METHODs ###############################################################################################
	def evaluate_single_graph(self, g: CompositeGraphObject, class_weights: Union[int, float, list[float]], training: bool) -> tuple:
	...
	# -----------------------------------------------------------------------------------------------------------------
	def evaluate(self, g: Union[CompositeGraphObject, list[CompositeGraphObject]], class_weights: Union[int, float, list[float]]) -> tuple:
		""" return ALL the metrics in self.extra_metrics + Iter & Loss for a CompositeGraphObject or a list of CompositeGraphObjects
		:param g: element/list of CompositeGraphObject to be evaluated
		...
		"""
		# chech if inputs are CompositeGraphObject OR list(s) of CompositeGraphObject(s)
		if not (type(g) == CompositeGraphObject or (type(g) == list and all(isinstance(x, CompositeGraphObject) for x in g))):
			raise TypeError('type of param <g> must be CompositeGraphObject or list of CompositeGraphObjects')
		if type(g) == CompositeGraphObject: g = [g]
	...
	## TRAINING METHOD ################################################################################################
	def train(self, gTr: Union[CompositeGraphObject, list[CompositeGraphObject]], epochs: int, gVa: Union[CompositeGraphObject, list[CompositeGraphObject], None] = None,
			  update_freq: int = 10, max_fails: int = 10, class_weights: Union[int, list[float]] = 1,
			  *, mean: bool = True, serial_training : bool = False, verbose: int = 3) -> None:
		""" TRAIN PROCEDURE
		:param gTr: CompositeGraphObject or list of CompositeGraphObjects used for the learning procedure
		...
		# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
		def checktype(elem: Optional[Union[CompositeGraphObject, list[CompositeGraphObject]]]) -> list[CompositeGraphObject]:
			""" check if type(elem) is correct. If so, return None or a list of CompositeGraphObjects """
			if elem is None: pass
			elif type(elem) == CompositeGraphObject: elem = [elem]
			elif isinstance(elem, (list, tuple)) and all(isinstance(x, CompositeGraphObject) for x in elem): elem = list(elem)
			else: raise TypeError('Error - <gTr> and/or <gVa> are not CompositeGraphObject or LIST/TUPLE of CompositeGraphObjects')
			return elem
		...
		for e in range(initial_epoch, epochs):
			...
			if e % update_freq == 0:
				...
				for i, j, namespace in zip(*self.get_weights(), self.namespace):
					for k in range(len(i)): self.write_net_weights(netS_writer, namespace, 'NetState'+str(k), i[k], e)
					self.write_net_weights(netO_writer, namespace, 'NetOutput', j, e)
	...
	## TEST METHOD ####################################################################################################
	def test(self, gTe: Union[CompositeGraphObject, list[CompositeGraphObject]], *, acc_classes: bool = False, rocdir: str = '',
			 micro_and_macro: bool = False, prisofsdir: str = '') -> dict[str, list[float]]:
		""" TEST PROCEDURE
		:param gTe: element/list of CompositeGraphObjects for testing procedure
		...
		"""
		if type(gTe) != CompositeGraphObject and not (type(gTe) == list and all(isinstance(x, CompositeGraphObject) for x in gTe)):
			raise TypeError('type of param <gTe> must be CompositeGraphObject or list of CompositeGraphObjects')
	...
	def LKO(self, dataset: Union[list[CompositeGraphObject], list[list[CompositeGraphObject]]], number_of_batches: int = 10, useVa: bool = False,
		...
		:param dataset: (list) of CompositeGraphObject OR (list) of lists of CompositeGraphObject on which <gnn> has to be valuated
		...
		:param node_aggregation: (str) for node aggregation method during dataset creation. See CompositeGraphObject for details
		...
		dataset = [CompositeGraphObject.merge(i, node_aggregation=node_aggregation) for i in flattened]
